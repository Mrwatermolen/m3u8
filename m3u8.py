...
def make_torrent(save_path, m3u8_file = None):
    if DEBUGMODE:
        print(f"m3u8_file:{m3u8_file} is os.path.exists {os.path.exists(m3u8_file)}\n")
    ...
    command = f"mktorrent -l 22 -p -v -o {os.path.join(save_path, torrent_name)} {save_path}"
    os.system(command)
...
if __name__ == '__main__':
    ...
    if  (not isFailed) and (input("input -1 to exit make torrent") != "-1"):
        if input("input 1 to make torrent for a file but dir") == "1":
            make_torrent(save_path=os.path.join(save_path, vedio_name + ".mp4"), m3u8_file=os.path.join(save_path, m3u8_file))
        make_torrent(save_path)
